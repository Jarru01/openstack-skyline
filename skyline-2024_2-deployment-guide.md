# Skyline Dashboard 2024.2 — Complete Source Deployment Guide
## Ubuntu 22.04 LTS (LXD Container), Single-Node (APIServer + Console + DB)

> **Target versions:** `skyline-apiserver` (stable/2024.2) · `skyline-console` (stable/2024.2)
> **OS:** Ubuntu 22.04 LTS (Jammy) in an LXD container
...
0. [Launching an LXC Container on Ubuntu 22.04 LTS](#0-launching-an-lxc-container-on-ubuntu-2204-lts)
...
## 0. Launching an LXC Container on Ubuntu 22.04 LTS

This section covers installing LXD, creating a container for Skyline, and attaching it to a management bridge. Run all commands in this section on the **host machine** (not inside a container).

### 0.1 Install LXD via snap

On Ubuntu 22.04, LXD is distributed as a snap package. The `5.x` channel is the current stable branch.

```bash
sudo snap install lxd --channel=5.x/stable
sudo snap refresh lxd
```

Add your user to the `lxd` group so you can manage containers without `sudo`:
...
Run the interactive initialisation. For a simple single-host setup, accept the defaults. Answer **yes** to the question about creating a new storage pool, and **no** to clustering.

```bash
lxd init
```

A minimal non-interactive initialisation (suitable for scripted setups):

```bash
lxd init --auto
```

### 0.3 Verify LXD is working

```bash
lxc list
# Should return an empty table with no error
```

### 0.4 Launch the Skyline development container

Launch a container named `skyline-dev` based on the Ubuntu 22.04 (Jammy) image:

```bash
lxc launch ubuntu:22.04 skyline-dev
```

LXD will download the image on first use. When complete, confirm the container is running:

```bash
lxc list
# NAME          STATE    IPV4           ...
# skyline-dev   RUNNING  10.x.x.x (eth0) ...
```

### 0.5 Attach the container to a management bridge and restart

If your environment uses a dedicated management bridge (e.g. `brMGMT`), add a bridged NIC to the container and restart it so the interface is live:

```bash
sudo lxc config device add skyline-dev eth0 nic nictype=bridged parent=brMGMT name=eth0
sudo lxc restart skyline-dev
```

> **Note:** After restart, the container will obtain an IP from the `brMGMT` bridge via DHCP (or a static assignment depending on your network config). Verify the new IP with `lxc list` before proceeding.

### 0.6 Enter the container

```bash
lxc exec skyline-dev -- bash
```

All steps from section 3 onwards (except the OpenStack controller steps) are run **inside this container**.
...
The official upstream docs use `sudo pip3 install ... --break-system-packages`. On Ubuntu 22.04 (Jammy) this pollutes system Python. **This guide uses a dedicated virtualenv** at `/opt/skyline-venv` instead. This is also how a proper Juju charm should manage Python applications.

> **Ubuntu 22.04 note:** PEP 668 is not enforced on Jammy (that restriction arrived in 24.04), but using a virtualenv is still best practice and is required for charm compatibility.

### 2.2 Node.js version (most likely to age poorly)

The upstream 2024.2 install guide pins `lts/gallium` (Node 16). Node 16 reached end-of-life in September 2023. It may still build successfully because the skyline-console JavaScript code has not changed its Node requirement in recent cycles, but you should **verify the actual engines field** in the repo before assuming it forever:
...
### 2.10 `libgconf-2-4` available on Ubuntu 22.04

Unlike Ubuntu 24.04, the package `libgconf-2-4` **is** available on Ubuntu 22.04 (Jammy) and **should be included** in the build dependency list for skyline-console. This guide lists it in section 7.1.

### 2.11 Default Python version on Ubuntu 22.04

Ubuntu 22.04 ships with **Python 3.10** as the default. All venv paths will therefore use `python3.10` (e.g. `/opt/skyline-venv/lib/python3.10/site-packages/`). This differs from 24.04 which uses Python 3.12. Adjust any commands referencing a version-specific path accordingly.
...
### 6.1 Clone the stable/2024.2 branch
...
### 7.1 Install system build dependencies (Ubuntu 22.04 specific)
...
> **Ubuntu 22.04 note:** Use `libasound2` (not `libasound2t64` — that variant is specific to Ubuntu 24.04). The package `libgconf-2-4` **is** available on Ubuntu 22.04 and **must** be included; it was only removed in Ubuntu 24.04.
...
Based on the documented requirements for 2024.2, the expected Node LTS is `gallium` (Node 16). If the output shows a different version requirement, install that version instead of the one below.
...
```
/opt/skyline-venv/lib/python3.10/site-packages/skyline_console/static
```
